...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
{
    ...
    case ESP_SPP_CLOSE_EVT:
        ...
        spp_handle = 0; //delete open hanlde
        break;
...
//receive 
    case ESP_SPP_DATA_IND_EVT:
        ...
        uint8_t ln[] = {'\n', 8}; //print skip line
        esp_spp_write(param->data_ind.handle, 2, ln);

        ch = param->data_ind.data[0];
    ...
    case ESP_SPP_SRV_OPEN_EVT:
        ...
        spp_handle = param->srv_open.handle; //copie handle
    ...
    while(true)
    {
        if(spp_handle != 0)
        {
            ...
            { 
                ...
                //write data spp
                if(u1_rx_buff_data_index > 0 && spp_handle != 0)
                {
                    esp_spp_write(spp_handle, u1_rx_buff_data_index, u1_rx_buff_data);
                    uint8_t ln[] = {'\n', 8}; //print skip line
                    esp_spp_write(spp_handle, 2, ln);
                }
                ch = '\0';
            }
        }
        vTaskDelay(pdMS_TO_TICKS(10));// el watchdog se paniquea sin el delay
    }
}
